...
static void caelispect_handleURL(NSURL *url)
{
...
static void caelispect_scene_openURLContexts(id self, SEL _cmd, UIScene *scene, NSSet<UIOpenURLContext *> *contexts)
{
    ...
    if (@available(iOS 13.0, *)) {
        for (UIOpenURLContext *context in contexts) {
            caelispect_handleURL(context.URL);
        }
    }
}

@interface CaelispectSceneDelegateHooks : NSObject
@end

@implementation CaelispectSceneDelegateHooks
...
+ (void)load
{
    ...
    if (method) {
        ...
        method_setImplementation(method, reinterpret_cast<IMP>(caelispect_scene_openURLContexts));
    } else {
        const char *types = "v@:@@";
        class_addMethod(cls, selector, reinterpret_cast<IMP>(caelispect_scene_openURLContexts), types);
    }
}
