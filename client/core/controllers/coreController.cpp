...
void CoreController::initAppleController()
{
    ...
    QTimer::singleShot(0, this, [this]() { Caelispect::toggleScreenshots(m_appSettingsRepository->isScreenshotsEnabled()); });
#endif
}
...
void CoreController::updateTranslator(const QLocale &locale)
{
    ...
    QDirIterator it(":/translations", QStringList("caelispect_*.qm"), QDir::Files);
    while (it.hasNext()) {
    ...
    const QString translationFilePrefix = QString(":/translations/caelispect_") + lang;
    QString strFileName = QString(":/translations/caelispect_%1.qm").arg(locale.name());
    ...
    if (m_translator->load(strFileName)) {
        ...
        if (m_translator->load(QString(":/translations/caelispect_en.qm"))) {
            QCoreApplication::installTranslator(m_translator);
        }
    }
