...
echo "[Caelispect] Installer package: $INSTALLER_PATH"

if [ ! -f "$INSTALLER_PATH" ]; then
    echo "[Caelispect] ERROR: Installer package not found: $INSTALLER_PATH"
    exit 1
fi

PKG_PATH="$INSTALLER_PATH"
echo "[Caelispect] Using PKG: $PKG_PATH"
...
echo "[Caelispect] Running installer..."
OSA_CMD='do shell script "/usr/sbin/installer -pkg '"$PKG_PATH"' -target /" with administrator privileges'
osascript -e "$OSA_CMD"

STATUS=$?
if [ $STATUS -ne 0 ]; then
    echo "[Caelispect] ERROR: installer exited with status $STATUS"
    exit $STATUS
fi

echo "[Caelispect] Cleaning up..."
rm -f "$INSTALLER_PATH" || true
rm -rf "$EXTRACT_DIR" 2>/dev/null || true

echo "[Caelispect] Installation completed successfully"
exit 0
