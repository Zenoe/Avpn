...
bool KillSwitch::enablePeerTraffic(const QJsonObject &configStr) {
    ...
    config.m_primaryDnsServer = configStr.value(caelispect::configKey::dns1).toString();

    // We don't use secondary DNS if primary DNS is CaelispectDNS
    if (!config.m_primaryDnsServer.contains(caelispect::protocols::dns::caelispectDnsIp)) {
        config.m_secondaryDnsServer = configStr.value(caelispect::configKey::dns2).toString();
    }

    config.m_serverPublicKey = "wireguard";
    ...
    for (const QJsonValue &i : configStr.value(caelispect::configKey::splitTunnelApps).toArray()) {
        if (!i.isString()) {
            break;
        }
    ...
    for (auto dns : configStr.value(caelispect::configKey::allowedDnsServers).toArray()) {
        if (!dns.isString()) {
            break;
        }
    ...
    // killSwitch toggle
    if (QVariant(configStr.value(caelispect::configKey::killSwitchOption).toString()).toBool()) {
...
bool KillSwitch::enableKillSwitch(const QJsonObject &configStr, int vpnAdapterIndex) {
    ...
    dnsServers.append(configStr.value(caelispect::configKey::dns1).toString());

    // We don't use secondary DNS if primary DNS is CaelispectDNS
    if (!configStr.value(caelispect::configKey::dns1).toString().contains(caelispect::protocols::dns::caelispectDnsIp)) {
        dnsServers.append(configStr.value(caelispect::configKey::dns2).toString());
    }
